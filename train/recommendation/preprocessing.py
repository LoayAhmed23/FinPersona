...
def fill_missing_values(profile):
    ...
    mcc_cols = [col for col in profile.columns if col.startswith('MCC_') and col.endswith('_SPEND')]
...
def one_hot_encode_categoricals(profile):
    ...
    if 'AGE_GROUP' in profile.columns:
        age_group_dummies = pd.get_dummies(profile['AGE_GROUP'], prefix='AGE_GROUP', drop_first=False)
    ...
    if 'GENDER' in profile.columns:
        gender_dummies = pd.get_dummies(profile['GENDER'], prefix='GENDER', drop_first=False)
        profile = pd.concat([profile, gender_dummies], axis=1)
        profile = profile.drop(columns=['GENDER'])

    if 'BRANCH_ID' in profile.columns:
        branch_id_dummies = pd.get_dummies(profile['BRANCH_ID'], prefix='BRANCH_ID', drop_first=False)
...
def drop_unneeded_columns(profile, prime_only=False, txn_only=False, final_stage=False):
    ...
    if prime_only:
        existing_cols_to_drop = [col for col in config.PRIME_COLUMNS_TO_DROP if col in profile.columns]
        profile = profile.drop(columns=existing_cols_to_drop)
    if txn_only:
        existing_cols_to_drop = [col for col in config.TRANSACTION_COLUMNS_TO_DROP if col in profile.columns]
        profile = profile.drop(columns=existing_cols_to_drop)
    if final_stage:
        final_columns_to_drop = ['AGE', "BRANCH_NAME", "PRODUCT_NAME", "DOB", "RIMNO", "DOB_WAS_MISSING", "GENDER_Unknown"]
        existing_cols_to_drop = [col for col in final_columns_to_drop if col in profile.columns]
        profile = profile.drop(columns=existing_cols_to_drop)
...
def filter_uncorrelated_features(profile, logs):
    ...
    features_to_keep = max_corr_per_feature[max_corr_per_feature >= config.CORR_THRESHOLD].index.tolist()
    features_to_drop = max_corr_per_feature[max_corr_per_feature < config.CORR_THRESHOLD].index.tolist()
...
def drop_low_volume_products(profile, logs):
    ...
    current_product_cols = [col for col in profile.columns if col.startswith('HAS_PROD_')]
...
def preprocess_pipeline(profile, logs):
    ...
    profile = drop_unneeded_columns(profile, prime_only=True, txn_only=True, final_stage=True)
    ...
    final_features = [col for col in profile.columns if col not in final_products and col != 'CUSTOMER_ID']
