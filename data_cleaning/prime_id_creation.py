...
def run(input_dir=None, output_dir=None):
    ...
    if not prime_files:
        print(
            f"ERROR: No CSV files found in '{input_dir}'. Make sure the folder exists."
        )
        return
    ...
    for file in prime_files:
        ...
        temp_df = pd.read_csv(
            file, encoding="latin", usecols=["RIM_NO", "DOB"], dtype="string"
        )
    ...
    global_pairs = (
        pd.concat(all_pairs, ignore_index=True).drop_duplicates().reset_index(drop=True)
    )
    ...
    for file in prime_files:
        ...
        existing_cols_to_drop = [
            col for col in prime_columns_to_drop if col in df.columns
        ]
        ...
        if unmatched > 0:
            print(
                f"  WARNING: {unmatched} rows could not be assigned a CUSTOMER_ID (missing RIMNO or DOB)."
            )
        ...
        dup_check = active_df.groupby(["RIMNO", "PRODUCT_NAME"])[
            "CUSTOMER_ID"
        ].nunique()
        ...
        if len(dup_groups) > 0:
            ...
            oldest_dob = (
                active_df[
                    active_df.set_index(["RIMNO", "PRODUCT_NAME"]).index.isin(
                        dup_groups
                    )
                ]
            ...
            active_df = active_df.merge(
                oldest_dob, on=["RIMNO", "PRODUCT_NAME"], how="left"
            )

            is_dup_group = active_df.set_index(["RIMNO", "PRODUCT_NAME"]).index.isin(
                dup_groups
            )
        ...
        if len(relatives_df) > 0:
            relatives_path = os.path.join(
                output_dir, f"{file_basename}_active_relatives.csv"
            )
